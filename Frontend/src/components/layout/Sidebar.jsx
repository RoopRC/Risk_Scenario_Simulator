...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <aside className={`fixed left-0 top-0 bottom-0 w-[220px] bg-gradient-to-b from-[#222831] via-[#222831] to-[#1a1e25] flex flex-col z-50 shadow-sidebar transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
      {/* Logo + Close */}
      <div className="px-5 py-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#948979] to-[#7d7466] flex items-center justify-center shadow-glow">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#DFD0B8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
          </div>
          <div>
            <h1 className="text-[#DFD0B8] font-bold text-base leading-tight tracking-wide">RISK.SIM</h1>
            <p className="text-[#948979]/60 text-2xs uppercase tracking-[0.2em] font-medium">Enterprise Edition</p>
          </div>
        </div>
        {/* Close button - mobile only */}
        <button onClick={onClose} className="md:hidden text-[#948979] hover:text-[#DFD0B8] p-1">
      ...
      <div className="px-5 mt-5 mb-3">
        <p className="text-[#948979]/50 text-2xs font-semibold uppercase tracking-[0.2em]">Systems</p>
      </div>
      ...
      <div className="px-4 mb-4">
        <div className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.04]">
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-2xs font-bold text-[#DFD0B8]/80 uppercase tracking-wider">Health Index</span>
            <span className="w-2 h-2 rounded-full bg-[#6B8F71] shadow-[0_0_6px_rgba(107,143,113,0.6)]"></span>
          </div>
          <div className="w-full h-1 bg-white/[0.06] rounded-full overflow-hidden mb-2">
            <div className="h-full w-[88%] bg-gradient-to-r from-[#6B8F71] to-[#948979] rounded-full transition-all duration-1000"></div>
          </div>
          <p className="text-2xs text-[#948979]/60">88% Service Availability</p>
        </div>
      </div>

      {/* User */}
      <div className="px-4 py-4 border-t border-white/[0.04] flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-[#948979]/15 flex items-center justify-center ring-1 ring-[#948979]/20">
          <span className="text-[#948979] font-bold text-xs">
            {(user?.name || user?.username || 'A').charAt(0).toUpperCase()}
          </span>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[#DFD0B8] text-sm font-medium truncate">{user?.name || user?.username || 'admin'}</p>
          <p className="text-[#948979]/60 text-2xs">{user?.role || 'ADMIN'}</p>
        </div>
        <button onClick={handleLogout} className="text-[#948979]/60 hover:text-[#C44536] transition-colors p-1 rounded-md hover:bg-white/[0.03]" title="Logout">
